...
/**
 * Tab-aware default sort: each filter has its own ordering logic.
 */
...
/**
 * User-selected sort overrides the default tab-aware sort.
 */
function applySortOption(clients, sortOption) {
  const sorted = [...clients];
  switch (sortOption) {
    case 'aum_asc':
      sorted.sort((a, b) => a.aum - b.aum);
    ...
    case 'aum_desc':
      ...
      break;
    case 'holdings_asc':
      sorted.sort((a, b) => a.holdings_count - b.holdings_count || b.aum - a.aum);
      break;
    case 'holdings_desc':
      sorted.sort((a, b) => b.holdings_count - a.holdings_count || b.aum - a.aum);
      break;
    default:
      return clients; // no override, keep input order
  }
  return sorted;
}
...
// ------------------------------------------------------------
export async function getClientList({ salesCode, priorityFilter = 'ALL', sortOption = 'default', page = 1, pageSize = 20 }) {
  // ========================================================
  ...
  // Apply user-chosen sort if any, otherwise tab-aware default
  if (sortOption && sortOption !== 'default') {
    clients = applySortOption(clients, sortOption);
  } else {
